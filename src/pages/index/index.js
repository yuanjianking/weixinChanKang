const constantData = require('../../utils/constantData.js');
// 右侧每一类的 bar 的高度（固定）
...
Page({
  ...
  onLoad: function () {
    ...
    this.setData({
      ...
      currentLeftSelect: constantData[0].id,
  ...
  getEachRightItemToTop: function () {
    ...
    var totop = 0;
    // 右侧第一类肯定是到顶部的距离为 0
    obj[constantData[0].id] = totop
    // 循环来计算每个子类到顶部的高度
    for (let i = 1; i < (constantData.length + 1); i++) {
      totop += (RIGHT_BAR_HEIGHT + constantData[i - 1].category.length * RIGHT_ITEM_HEIGHT)
      // 这个的目的是 例如有两类，最后需要 0-1 1-2 2-3 的数据，所以需要一个不存在的 'last' 项，此项即为第一类加上第二类的高度。
      obj[constantData[i] ? constantData[i].id : 'last'] = totop
    }
    return obj
  },
  //记录swiper滚动的
